...
class HomeController extends GetxController {
  ...
  Future<void> getFanProfile() async {
    try {
      // Passa onesignal_player_id per recovery fan code se device_id è cambiato
      final playerId = OneSignal.User.pushSubscription.id ?? '';
      final response = await http.get(
        Uri.parse('$fanApiUrl?device_id=$_deviceId&onesignal_player_id=$playerId&_t=${DateTime.now().millisecondsSinceEpoch}'),
        headers: {'Cache-Control': 'no-cache', 'Pragma': 'no-cache'},
      ).timeout(const Duration(seconds: 10));
